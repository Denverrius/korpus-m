...
def handle_start(message):
    ...
    if is_admin(uid, uname):
        text = (
            ...
            f"• 🎁 <b>Акции & 🍲 Меню</b>: Управление каталогом.\n\n"
            f"<i>Используйте кнопки ниже для перехода:</i>"
        )
        bot.send_message(message.chat.id, text, parse_mode="HTML", reply_markup=get_admin_inline_keyboard())
        bot.send_message(message.chat.id, "Клавиатура управления внизу экрана ⬇️", reply_markup=get_admin_reply_keyboard())
    else:
        text = (
...
def handle_all_messages(message):
    ...
    if is_admin(uid, uname):
        if "Акции на сайте" in text:
            ...
            bot.send_message(message.chat.id, "Выберите действие в меню:", reply_markup=get_admin_inline_keyboard())
    else:
...
if __name__ == "__main__":
    try:
        ...
        pass
    logging.info("🍲 Restaurant «На Бульваре» Bot (@na_bulvare_bot) started polling on Railway...")
